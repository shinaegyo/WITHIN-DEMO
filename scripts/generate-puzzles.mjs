...
const ROUNDS = 3;
const DAYS = Math.min(Number(arg('days', 333)), Math.floor(MAX / ROUNDS));
const SEED = Number(arg('seed', 20260813));
const OUT = arg('out', 'supabase/migrations/0011_seed_rounds.sql');
...
for (let day = 0; day < DAYS; day += 1) {
  ...
  for (let round = 1; round <= ROUNDS; round += 1) {
    // Draws straight off the shuffled pool, so no number repeats across the
    // whole run — not just within a day.
    const answer = order[day * ROUNDS + (round - 1)];
    ...
    rows.push({ date: iso, round, answer, clue1: clues.c1.text, clue2: clues.c2.text });
  }
}
...
-- start=${START} days=${DAYS} rounds=${ROUNDS} seed=${SEED}
...
console.log(`wrote ${rows.length} rounds across ${days.size} days to ${OUT}`);
...
console.log('\nfirst day:');
for (const r of rows.slice(0, 3)) console.log(`  ${r.date}  R${r.round}  ${String(r.answer).padStart(4)}  "${r.clue1}" / "${r.clue2}"`);
