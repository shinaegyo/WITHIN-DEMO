-- Fixes submit_guess.
--
-- round_results.status is an enum but the CASE assigning it produces text, and
-- Postgres refused the update rather than coercing: every guess failed with
-- "column status is of type round_status but expression is of type text".
-- The score column needed the same for smallint.
