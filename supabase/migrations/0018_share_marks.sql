-- Include each round's guess pattern in game_state.
--
-- The share card needs a square per guess for every round, but state only
-- carried the guesses of the round being played. Directions are enough: they
-- say whether each guess was high, low or right, without exposing a number.
--
-- Sharing is safe precisely because round order is per player — a friend's
-- "round 1" is not yours, so their pattern tells you nothing about your own.
...
begin
  ...
  return jsonb_build_object(
    'puzzleDate',   v_date,
    -- Days since launch, so a share can say which puzzle it was.
    ...
    'rounds', coalesce((
      select jsonb_agg(jsonb_build_object(
               ...
               -- One entry per guess: 'below', 'above' or 'correct'.
               'marks', coalesce((
