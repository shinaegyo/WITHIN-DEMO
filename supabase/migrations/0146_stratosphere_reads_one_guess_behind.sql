-- Stratosphere reads one guess behind.
--
-- Levels 31 to 45 were Sky without a clue and nothing else - the same question
-- as the fifteen below and the fifteen above, at a slightly higher price. A
-- tier should change what a level is, not what it costs.
--
-- So the colour arrives late. Every guess reports its direction at once and
-- its shade only once another guess has landed on top of it. No information is
-- removed; it is simply always one step behind, so a player commits to the
-- next guess before learning how close the last one was.
--
-- Two things it deliberately is not. It is not fewer attempts - "blind first
-- guess" was the other candidate and on inspection it is five attempts minus
-- one with an extra tap, because a guess that returns nothing is a guess you
-- throw away, and four attempts is a 93% failure rate. And it is not less
-- information, which is what stripping bands would have been; the shade always
-- comes, just not yet.
--
-- A correct guess never withholds. The level is over.

begin;

/** Which levels hold their colour back a step. Stratosphere, and only it. */
...
/**
 * Whether this particular tile is still owed its colour: the newest guess on
 * a delaying level, and not a winning one.
 */
create or replace function public.endless_pending(
  p_run public.endless_runs, p_index integer, p_direction text
)
returns boolean
language sql
immutable
as $fn$
  select public.endless_delays_colour(p_run.level)
     and p_index = p_run.attempts_used
     and p_direction <> 'correct';
$fn$;
...
revoke execute on function public.endless_pending(public.endless_runs,integer,text)
