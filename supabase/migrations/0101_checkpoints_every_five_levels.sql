-- Checkpoints every five levels, not every twenty.
--
-- Somebody reached level 16, came back the next morning and was on level 1.
-- That is not a bug: the checkpoint is the floor of the deepest TIER reached,
-- The Shallows runs 1 to 19, and a new day starts at the checkpoint. Fifteen
-- numbers of work, correctly thrown away by a rule working exactly as written.
--
-- The rule is wrong at the shallow end. Twenty levels is an hour of good play
-- and losing it is not a setback, it is a reason to stop opening the app - and
-- it lands hardest on new players, who are the only ones down there.
--
-- So the tiers keep doing what tiers are for - naming the depth, setting the
-- attempts, paying 50 XP when you break into a new one - and they stop being
-- the checkpoints. A checkpoint is every fifth level. Whatever happens, a day
-- begins at most four numbers behind where the last one ended, at level 16 or
-- at level 96.
--
-- endless_climb and endless_guess are 0089's, extracted verbatim, with the two
-- checkpoint calls swapped and nothing else touched. The XP bonus still asks
-- arena_floor, because that one really is about tiers.
...
immutable
as $$
  select greatest(1, (coalesce(p_level, 1) / 5) * 5)::smallint;
$$;
