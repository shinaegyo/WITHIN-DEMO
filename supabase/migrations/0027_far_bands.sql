-- Split the far end of the proximity scale into three.
--
-- Everything beyond 99 collapsed into one band, so a guess 100 away and a guess
-- 900 away looked identical. That is the widest part of the range and the part
-- a player opens with, and it was the part carrying the least information: the
-- opening guess told you almost nothing beyond a direction.
--
-- The close bands are untouched. They are where the game is decided, and their
-- widths were tuned around that.
--
--   500+      vast      new
--   250-499   distant   new
--   100-249   light     was "100 or more"
--   25-99     medium    unchanged
--   11-24     dark      unchanged
--   1-10      intense   unchanged
--
-- Guesses already recorded keep the tier they were given. An old 'light' could
-- have been any distance over 99, so historical rows read as the coarser scale
-- they were played on — which is honest, and only visible on a round still in
-- progress when this runs.

alter table public.guesses drop constraint if exists guesses_tier_check;

alter table public.guesses
  add constraint guesses_tier_check
  check (tier in ('vast', 'distant', 'light', 'medium', 'dark', 'intense', 'correct'));
